...
define("moxman/interop/TinyMcePlugin", [
	...
	tinymce.PluginManager.add('moxiemanager', function(editor, url) {
		...
		var tinyMCEVersion = parseInt(tinymce.majorVersion, 10) || 3;
		...
		function getZIndex() {
			switch (tinyMCEVersion) {
				case 3:
					return editor.windowManager.zIndex;
				case 4:
					return tinymce.ui.FloatPanel.currentZIndex;
				case 5:
					// Oxide uses 1000-10000 as the z-index, so use that as the range
					return 10000;
				default:
					return 0;
			}
		...
		function browse(value, type, callback) {
			var zIndex = getZIndex(), url;
		...
		function file_browser_callback(id, value, type, win) {
			browse(value, type, function(args) {
		...
		function file_picker_callback(callback, value, meta) {
			browse(value, meta.filetype, function(args) {
				callback(convertUrl(args.focusedFile.meta.url));
			});
		}

		if (tinyMCEVersion >= 5) {
			editorSettings.file_picker_callback = file_picker_callback;
		} else {
			editorSettings.file_browser_callback = file_browser_callback;
		}
		...
		if (tinyMCEVersion >= 5) {
			...
			editor.ui.registry.addButton('insertfile', {
				...
				title: 'Insert file',
				onAction: function () {
					editor.execCommand('mceInsertFile');
				}
			});
		} else if (tinyMCEVersion === 4) {
			// TinyMCE 4.x+
